...
## Credential
HOST = "10.165.67.146"  # MySQL server host DNS
PORT = 3306  # MySQL server port number
USER = "robertclaud"  # MySQL account name
PASSWORD = "Qwer1234!"  # Password of the account
DB = "ME597Spring26O"  # DB name
TABLE = "robertclaud_groupproject"  # table name
## Credential
...
MTCONNECT_STR = (
    ET.fromstring(requests.get(url_current).content).tag.split("}")[0] + "}"
)
print("MTConnect string header is {}.".format(MTCONNECT_STR))
## MTConnect info.

url = (
    "http://" + agent + ":" + agent_port + "/sample"
)  # sample request to MTConnect agent
...
cursor = connection.cursor()  # Open cursor to execute SQL Query
cursor.execute(
    "SET time_zone='+00:00'"
)  # Set time zone of the connected session as UTC
...
while True:  # reapeating continuously, infinitely
    try:
        ...
        for device in root.iter(MTCONNECT_STR + "DeviceStream"):
            ...
            for sample in device.iter(
                ...
                for tags in sample:
                    ...
                    timestamp_reformat = (
                        timestamp[0:10] + " " + timestamp[11:]
                    )  # reformat timestmap according to MySQL
                    timestamp_mysql = timestamp_reformat.replace(
                        "Z", ""
                    )  # remove Z in timestamp
                    # print(timestamp, timestamp_mysql)
                    name = tags.get("name")  # get name info
                    if (
                        name is None or name == " "
                    ):  # in case name attribute is empty
                        name = "NULL"
                    ...
                    query = (
                        "INSERT INTO "
                        + TABLE
                        + " (timestamp,sensor,measurement,value) VALUE('"
                        + timestamp_mysql
                        + "','"
                        + deviceName
                        + "','"
            ...
            for event in device.iter(
                MTCONNECT_STR + "Events"
            ):  # for Events category
                for tags in event:
                    tagName = tags.tag  # get tag info
                    tagName = tagName[40:]  # tagname
        ...
        url_updated = (
            url + "?from=" + nextSeq
        )  # update url for the next sequence
        print("== Collection is DONE ==: Next sequence is", nextSeq, "\n")
        time.sleep(
            3
        )  # 3 seconds time sleep.CHANGE THIS depneding on your system requirement
