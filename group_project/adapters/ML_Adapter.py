...
class MTConnectAdapter(object): # MTConnect adapter object
    ...
    def adapter_stream(self, model, min_val, max_val, threshold):
        while True:
            try:
                x, y, z = measureData(acc,1000) # x=x-axis, y=y-axis, z-axis acceleration array
                input_feature = freqFeatures(y) # input feature for ML model, here we use y-axis frequency feature as an example, you can change it to other features if you want
...
if __name__ == "__main__":
    model_path = '/home/pi/Documents/lab/group_project/models/best_model'
    model = keras.layers.TFSMLayer(model_path, call_endpoint="serving_default")

    threshold = 0.06974803
    min_val = 0
    max_val = 0.3741513401613265
    
    # start MTConnect Adapter
    adapter = MTConnectAdapter('0.0.0.0', 7878) # Args: host ip, port number
    adapter.adapter_stream(model, min_val, max_val, threshold) # start streaming data to MTConnect, args: min, max for data normalization, and threshold for prediction
